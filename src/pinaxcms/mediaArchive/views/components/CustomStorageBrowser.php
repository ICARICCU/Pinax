<?php
class pinaxcms_mediaArchive_views_components_CustomStorageBrowser extends pinax_components_Component
{
    function init()
    {
...
// TODO localizzare l'interfaccia
        $this->_application->_rootComponent->addOutputCode( pinax_helpers_JS::JScode( 'if ( typeof(Pinax) == "undefined" ) Pinax = {}; Pinax.baseUrl ="'.PNX_HOST.'"; Pinax.ajaxUrl = "ajax.php?pageId='.$this->_application->getPageId().'&ajaxTarget='.$this->getId().'&onlyFolder='.$onlyFolder.'&customPath='.$customPath.'&enableDownload='.$enableDownload.'&command=";' ), 'head' );
...
EOD;
    ...
    function process_ajax()
    {
        ...
        if ( $command == "read" )
        {
            ...
            if (!$customPath) {
                $mappingService = $this->_application->retrieveProxy('pinaxcms.mediaArchive.services.MediaMappingService');
            ...
            if (!empty($files)) {
                ...
                foreach ($files as $file_name)
                {
                    ...
                    else if (!$onlyFolder)
                    {
                        ...
                        if ($enableDownload) {
                            $name = pinax_helpers_Link::makeSimpleLink($file_name, $fullPath, '', '', '', array('download' => $file_name));
                        } else {

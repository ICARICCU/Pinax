...
class pinaxcms_mediaArchive_media_Image extends pinaxcms_mediaArchive_media_Media
{
	var $_imageInfo = null;
	var $_cacheObj;
	...
	function getThumbnail($width, $height, $crop=false, $cropOffset=1, $forceSize=false, $useThumbnail = true, $returnResizedDimension = true )
	{
		...
		return array(	'imageType' 		=> NULL,
						'fileName' 			=> pinaxcms_helpers_Media::getImageUrlById($this->id, $width, $height, $crop, $cropOffset, $forceSize, $useThumbnail ),
						'width' 			=> $newWidth,
						'height' 			=> $newHeight,
						'originalWidth' 	=> NULL,
						'originalHeight' 	=> NULL);
	...
	function resizeImage($cacheFileName, $width, $height, $crop=false, $cropOffset=1, $forceSize=false, $usePiramidalSizes = true, $resize = true)
	{
		...
		$dirPath = pathinfo($cacheFileName, PATHINFO_DIRNAME);
	...
	private function resizeImage_gd($cacheFileName, $width, $height, $crop=false, $cropOffset=1, $forceSize=false, $usePiramidalSizes = true, $resize = true)
	{
		...
		if (!is_null($objImg))
		{
			...
			imageJPEG($img, $cacheFileName, __Config::get('JPG_COMPRESSION') );
			imageDestroy($objImg);
			imageDestroy($img);
			unset( $objImg );
			unset( $img );
		...
		{
			$fileName = pinax_Assets::get('ICON_MEDIA_IMAGE');
	...
	private function resizeImage_im($cacheFileName, $width, $height, $crop=false, $cropOffset=1, $forceSize=false, $usePiramidalSizes = true, $resize = true)
	{
		if ( file_exists( $this->getFileName() ) )
		{
    		...
    		$cacheFileName = str_replace(__Paths::get('CACHE'), pinax_Paths::getRealPath('CACHE'), $cacheFileName);
            $thumb = new Imagick();
            ...
            if($resize) {
                ...
                $thumb->setImageCompressionQuality(__Config::get('JPG_COMPRESSION'));
	            $thumb->stripImage();
            }
	...
	{
		$cacheOptions = array(
			'cacheDir' => pinax_Paths::get('CACHE_IMAGES'),
			'lifeTime' => __Config::get('CACHE_IMAGES'),
			'hashedDirectoryLevel' => __Config::get('CACHE_IMAGES_DIR_LEVEL'),
			'readControlType' => '',
			'fileExtension' => '.jpg'
		);

		$this->_cacheObj = &pinax_ObjectFactory::createObject('pinax.cache.CacheFile', $cacheOptions);

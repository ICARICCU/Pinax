...
class pinax_components_List extends pinax_components_HtmlFormElement
{
    var $_items;
    ...
    function process()
    {
        ...
        if ( !is_subclass_of( $this->_parent, 'pinax_components_RecordDetail' ) )
        {
            if ( !is_null( $linkedTo ) )
            {
            ...
            {
                $checkValuesClass =  &pinax_ObjectFactory::createObject($this->getAttribute('checkValues'));
...
EOD;
    ...
    function _render()
    {
        ...
        else
        {
            ...
            {
                $output .= pinax_helpers_Html::hidden( $attributes['name'], $this->_content );
            ...
            foreach($this->_items as $item)
            {
                ...
                $output .= '<option value="'.pinax_encodeOutput($item['key']).'"'.$selected.$disabled.$options.'>'.pinax_encodeOutput($item['value']).'</option>';
            }
            $output .= '</select>';
    ...
    public function render_form()
    {
        ...
        $attributes['title'] = $this->getAttributeString('title');
        $attributes['onchange'] = $this->getAttribute('onChange');
        $attributes['readOnly'] = $this->getAttribute('readOnly');
        $attributes['size'] = $this->getAttribute('rows');
        $attributes['multiple'] = $this->getAttribute('multiSelect') ? 'multiple' : '';
        $attributes['items'] = [];
        ...
        foreach($this->_items as $item)
        {
            if ( $this->_content=='' || is_null( $this->_content ) )
            {
                $selected = (isset($item['selected']) && $item['selected']) ? 'selected':'';
            }
            else
            {
                if ( is_array( $this->_content ) )
                {
                    $selected = in_array( strtoupper( $item['key'] ), $this->_content ) ? 'selected':'';
                }
                else
                {
                    $selected = strtoupper($item['key'])==strtoupper($this->_content) ? 'selected':'';
                }
            }
            $disabled = isset($item['disabled']) ? ' '.$item['disabled'] : '';
    ...
    {
        $this->_items[] = array('key' => $key, 'value' => $value, 'selected' => $selected, 'options' => null);
    }
}

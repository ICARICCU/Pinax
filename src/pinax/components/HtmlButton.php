...
class pinax_components_HtmlButton extends pinax_components_Component
{
	/**
	 * Init
	 *
	...
	function init()
	{
		...
		$this->defineAttribute('cssClass',	false, 	__Config::get('pinax.formButton.cssClass'),			COMPONENT_TYPE_STRING);
	...
	function render($outputMode = NULL, $skipChilds = false)
	{
		...
		{
			$attributes['value'] = pinax_encodeOutput($this->getAttribute('label'));
			$output .= '<input '.$this->_renderAttributes($attributes).' />';
		}
		else
		{
			$attributes['value'] = pinax_encodeOutput($this->getAttribute('value'));

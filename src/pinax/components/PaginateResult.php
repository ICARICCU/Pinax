...
if (!defined('PNX_PAGINATE_PAGE')) 		define('PNX_PAGINATE_PAGE', 'pageNum');

/**
 * Class pinax_components_PaginateResult
 */
class pinax_components_PaginateResult extends pinax_components_Component
{
	var $pageUrl;
	var $pageLength;
	...
	function setRecordsCount($recordsCount=NULL, $pageLength=NULL)
	{
		...
		$this->pageUrl = PNX_PAGINATE_PAGE;
	...
	function render($outputMode=NULL, $skipChilds=false)
	{
		if ($this->_content['totalPages']>1)
		{
			...
			if ($this->_content['totalPages']>$groupLength )
			{
				...
				if ($currentPage!=1 || $showDisabledLinks)
				{
					...
					if ($label) {
						...
						$tempArray['__url__'] = $currentPage != 1 ? pinax_helpers_Link::addParams(array($this->paramName => max(1, $currentPage - $pageJump))) : '';
				...
				if ($currentPage!=$this->_content['totalPages'] || $showDisabledLinks )
				{
					...
					if ($label) {
						$tempArray = array();
						$tempArray['__cssClass__'] = 'noNumber';
						$tempArray['__url__'] = $currentPage != $this->_content['totalPages'] ? pinax_helpers_Link::addParams(array($this->paramName => min($this->_content['totalPages'], $start + $pageJump))) : '';
						$tempArray['value'] = $label;
						$lastLink[] = $tempArray;
					}
				}
			...
			if (($currentPage!=1 || $showDisabledLinks) && $label)
			{
				$tempArray = array();
				$tempArray['__cssClass__'] = 'noNumber';
				$tempArray['__url__'] = pinax_helpers_Link::addParams(array($this->paramName => $currentPage-1));
				$tempArray['value'] = $label;
				$this->_content['pagesLinks'][] = $tempArray;
			}
			...
			for ($i = $start; $i<=$stop; $i++)
			{
				...
				if ($currentPage!=$i) $tempArray['__url__']	= pinax_helpers_Link::addParams(array($this->paramName => $i));
				$this->_content['pagesLinks'][]= $tempArray;
			}
		...
		{
			$form = '<form action="'.pinax_helpers_Link::addParams(array()).'" method="post">';
			$form .= '<input class="page" type="text" onfocus="this.value=\'\';" value="#" name="'.$this->paramName.'"/>';
			$form .= '<input class="go" type="image" src="'.pinax_Assets::get('ICON_GO').'"/>';
			$form .= '</form>';
	...
	function splitTextResult($records)
	{
		...
		$this->pageUrl = PNX_PAGINATE_PAGE; // TODO sostituire con un attributo
		$currentPage = $this->_sessionEx->get($this->pageUrl, '1', true, true); //pinax_Request::get($this->pageUrl, '1');
...
/**
 * Class pinax_components_PaginateResult_render
 */
class pinax_components_PaginateResult_render extends pinax_components_render_Render
{
...
<div tal:attributes="id PaginateResult/id;class PaginateResult/__cssClass__" tal:condition="php: count(PaginateResult['pagesLinks']) || !is_null(PaginateResult['totalRecords'])">
	<span tal:condition="php: !is_null(PaginateResult['totalRecords'])" >
		<span tal:omit-tag="" tal:content="php: __T('PNX_TOTAL_RECORDS')" /> <span tal:omit-tag="" tal:content="structure PaginateResult/totalRecords" />
	</span>
	<ul tal:condition="php: count(PaginateResult['pagesLinks'])" tal:attributes="class PaginateResult/__cssClass__">

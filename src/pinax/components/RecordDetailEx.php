...
class pinax_components_RecordDetailEx extends pinax_components_Component
{
	var $recordId;
	var $items = array();
	...
	function render($outputMode = NULL, $skipChilds = false)
	{
		...
		foreach( $this->items as $i )
		{
			...
			if ( !empty( $renderCell ) )
			{
				...
				{
					$renderCell = &pinax_ObjectFactory::createObject( $renderCell );
	...
	function getChildContent()
	{
		...
		if (count($this->childComponents))
		{
			for ($i=0; $i<count($this->childComponents);$i++)
			{
				...
				if ($this->childComponents[$i]->_tagname=='pnx:Groupbox')
				{
					// TODO
	...
	public static function compile($compiler, &$node, &$registredNameSpaces, &$counter, $parent='NULL', $idPrefix, $componentClassInfo, $componentId)
	{
		...
		{
			if ( $n->nodeName == "pnx:RecordDetailItem" )
			{

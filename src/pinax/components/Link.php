...
class pinax_components_Link extends pinax_components_Component
{
	/**
	 * Init
	 *
	...
	function render_html()
	{
		...
		if ( is_null( $url ) )
		{
			$output = pinax_helpers_Link::makeLink2($this->getAttribute('routeUrl'), $this->_content, array('class' => $cssClass, 'id' => $this->getId()));
		}
		else
		{
			$output = pinax_helpers_Link::makeSimpleLink( $this->_content['label'], $url, '', $cssClass, '', array('target' => $this->getAttribute('target'), 'icon' => $this->getAttribute('icon'), 'id' => $this->getId()) );
		}

...
class pinax_components_LongText extends pinax_components_Text
{
	/**
	 * Init
	 *
	...
	function getContent()
	{
		...
		if ($this->getAttribute('adm:htmlEditor')!==true)
		{
			...
			{
				$output = pinax_helpers_Html::forceP($output);
			}
			return $output;
		}
		else
		{
            ...
            if ($this->getAttribute('parseInternalLinks')) {
			    $output = pinax_helpers_Link::parseInternalLinks($output);
            }
	...
	{
		return str_replace( array("\r\n", "\n", "\r"), "<br />", pinax_encodeOutput( str_replace( '<br />', "\r\n", $this->getText() ) ) );
	}

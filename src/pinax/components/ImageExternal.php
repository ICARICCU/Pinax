...
class pinax_components_ImageExternal extends pinax_components_Component
{
	var $imageInfo;
	var $media;
	var $mediaUrl;
	...
	{
		$parent = $this->_parent->_tagname;
		if ($parent=='pnx:RecordSetList')
		{
		...
		if ( !empty( $this->mediaUrl ) )
		{
			...
			if (is_string($this->mediaUrl) && (!is_null($this->getAttribute('width')) || !is_null($this->getAttribute('height')))) {
			    if (strpos( $this->mediaUrl, 'http://') === 0 or strpos( $this->mediaUrl, 'https://') === 0) {
    			    ...
    			    $this->media = __ObjectFactory::createObject('pinaxcms.mediaArchive.media.Image', $params);
			    }
	...
	function attachMedia()
	{
		...
		{
			$this->media = &pinaxcms_mediaArchive_MediaManager::getEmptyMediaByType( 'IMAGE' );

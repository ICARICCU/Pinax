...
class pinax_components_JSTabGroup extends pinax_components_ComponentContainer
{
	/**
	 * Init
	 *
	...
	public function init()
	{
		...
		$this->defineAttribute('cssClass', false, __Config::get('pinax.jstab'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassPane', false, __Config::get('pinax.jstab.pane'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassDropdownTab', false, __Config::get('pinax.jstab.dropdown.tab'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassDropdown', false, __Config::get('pinax.jstab.dropdown.menu'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassDropdownLink', false, __Config::get('pinax.jstab.dropdown.link'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassDropdownCaret', false, __Config::get('pinax.jstab.dropdown.caret'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassNavigation', false, __Config::get('pinax.jstab.navigation'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('cssClassNavigationLink', false, __Config::get('pinax.jstab.navigation.link'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('iconNext', false, __Config::get('pinax.jstab.navigation.next'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('iconPrev', false, __Config::get('pinax.jstab.navigation.prev'), COMPONENT_TYPE_STRING);
		$this->defineAttribute('showNav', false, false, COMPONENT_TYPE_BOOLEAN);
		$this->defineAttribute('enableTabListener', false, __Config::get('pinax.jstab.enableTabListener'), COMPONENT_TYPE_BOOLEAN);
		$this->defineAttribute('enableScrollOnChange', false, __Config::get('pinax.jstab.enableScrollOnChange'), COMPONENT_TYPE_BOOLEAN);
		$this->defineAttribute('enableHash', false, __Config::get('pinax.jstab.enableHash'), COMPONENT_TYPE_BOOLEAN);
	...
	private function renderTabsNavigation()
	{
		...
		for ($i=0; $i<count($this->childComponents); $i++) {
			...
			if (!$visible || $this->childComponents[$i]->_tagname!='pnx:JSTab') continue;
	...
	private function renderJSCode()
	{
		$tabId = $this->getId();
		$tabIdContent = $tabId.'_content';
		$enableTabListener = $this->getAttribute('enableTabListener') === true ? 'true' : 'false';
		$enableScrollOnChange = $this->getAttribute('enableScrollOnChange') === true ? 'true' : 'false';
		$enableHash = $this->getAttribute('enableHash') === true ? 'true' : 'false';
		$tabCssClass = __Config::get('pinax.jstab.pane');

		$output = <<<EOD
<script>
jQuery(function(){
	var enableTabListener = {$enableTabListener};
	var enableScrollOnChange = {$enableScrollOnChange};
	var enableHash = {$enableHash};
	var isNested = $('#{$tabIdContent}').parent().closest('div.{$tabCssClass}').length != 0;

	if (Pinax && Pinax.events) {
	 	Pinax.events.on("pinax.message.showError", function() {

...
class pinax_Assets
{
	...
	{
		$basePath = pinax_Paths::get('CORE').'static/images/';
		$pathsArray 				= &pinax_Assets::_getPathsArray(true);
	...
	{
		$pathsArray = &pinax_Assets::_getPathsArray();
	...
	{
		$params[ 'label' ] = pinax_Assets::getImgTag( $iconName, $title );
		$params[ 'title' ] = $title;
		return pinax_helpers_Link::makeLink( $routing, $params, array(), $confirm, false );
	}
	...
	{
		$params[ 'label' ] = &pinax_Assets::getImgTag( $iconName, $title ).$title;
		$params[ 'title' ] = $title;
		return pinax_helpers_Link::makeLink( $routing, $params, array(), $confirm, $encode ).'<br />';
	}
	...
	public static function getImgActionWithTooltip( $iconName, $title, $description, $routing, $params, $confirm='' )
	{
		$params[ 'label' ] = pinax_Assets::getImgTag( $iconName, $title );
		$params[ 'title' ] = $title;
		return '<span title="'.$title.'|'.$description.'">'.pinax_helpers_Link::makeLink( $routing, $params, array(), $confirm ).'</span>';
	}
	...
	public static function getImgActionWithEnTooltip( $iconName, $title, $description )
	{
		static $t = 0;
		$t++;
		$output = '<a id="t'.$t.'" title="'.$title.'" rel="#td'.$t.'">'.pinax_Assets::getImgTag( $iconName, $title ).'</a>';
	...
	public static function &_getPathsArray($init=false)
	{
		...
		if (!count($_pathsArray) && !$init) pinax_Assets::init();
		return $_pathsArray;
	}
	...
	public static function makeLinkWithIcon($routeUrl, $iconName, $params, $deleteMsg=NULL )
	{
		...
		return pinax_helpers_Link::makeLink( $routeUrl, $params, array(), $deleteJs, false );
	}
}

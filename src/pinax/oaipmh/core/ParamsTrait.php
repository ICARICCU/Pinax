<?php
trait pinax_oaipmh_core_ParamsTrait
{
    ...
    private function getParam($name, $required, $type)
    {
        ...
        if ($required && !$exists) {
            throw pinax_oaipmh_core_Exception::missingArgument($name);
    ...
    {
        if (!pinax_oaipmh_OaiPmh::checkDateFormat($value) ) {
            throw pinax_oaipmh_core_Exception::badGranularity($name, $value);
        }
    ...
    private function validateResumptiontoken($name, $value)
    {
        if ($value && __Request::exists('from') && __Request::exists('until') && __Request::exists('set') && __Request::exists('metadataPrefix')) {
            throw pinax_oaipmh_core_Exception::exclusiveArgument();
        }

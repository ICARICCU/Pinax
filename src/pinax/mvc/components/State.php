...
class pinax_mvc_components_State extends pinax_components_ComponentContainer
{
	private $actions;
	...
	public static function compile($compiler, &$node, &$registredNameSpaces, &$counter, $parent='NULL', $idPrefix, $componentClassInfo, $componentId)
	{
		...
		$compiler->_classSource .= '$n'.$counter.'->setAddChildMethodName(\''.$stateName.'\', \''.$methodName.'\')'.PNX_COMPILER_NEWLINE;
		...
		foreach( $node->childNodes as $nc )
		{
			if ($nc->nodeName == 'mvc:includeState') {
				...
				$compiler->_classSource .= '// start mvc:includeState '.$nc->getAttribute('name').PNX_COMPILER_NEWLINE2;
				foreach ($elements as $nInState) {
					$counter++;
					$compiler->_compileXml($nInState, $registredNameSpaces, $counter, '$n'.$oldcounter, $idPrefix);
				}
				$compiler->_classSource .= '// end  mvc:includeState '.$nc->getAttribute('name').PNX_COMPILER_NEWLINE2;
				continue;
            }

...
class pinax_dataAccessDoctrine_ActiveRecord2tables extends pinax_dataAccessDoctrine_AbstractActiveRecord
{
    ...
    public function addField(pinax_dataAccessDoctrine_DbField $field, $isDetailField = false)
    {
        if ($isDetailField) {
        ...
        if ($field->key) {
            if (!$this->primaryKeyName && !$isDetailField) {
                ...
                throw pinax_dataAccessDoctrine_ActiveRecordException::detailPrimaryKeyAlreadyDefined($this->tableName);
            }
        }
    }
    ...
    public function createRecordIterator() {
        return new pinax_dataAccessDoctrine_RecordIterator2tables($this);
    }
    ...
    {
        $options = array_merge(['addFrom' => true, 'tableAlias' => 't1', 'tableDetailAlias' => 'detail'], $options);

        $qb = $this->connection->createQueryBuilder();

        if ($options['addFrom']) {
            $tableAlias = $options['tableAlias'];
            $tableDetailAlias = $options['tableDetailAlias'];

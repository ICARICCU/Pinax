...
class pinax_dataAccessDoctrine_RecordIteratorDocument extends pinax_dataAccessDoctrine_AbstractRecordIterator
{
    ...
    {
        if (isset($this->options['type']) && $this->options['type'] == 'PUBLISHED_DRAFT') {
            $options = array_merge(
                $this->options,
                array(
    ...
    {
        return (isset($this->options['type']) && $this->options['type'] == 'PUBLISHED_DRAFT') ? self::DOCUMENT_DETAIL_TABLE_PUBLISHED_ALIAS : self::DOCUMENT_DETAIL_TABLE_ALIAS;
    }
    ...
    public function selectDistinct($fieldName, $fields=[])
    {
        ...
        if ($this->ar->getDriverName() == 'mysql') {
            $this->qb->select('DISTINCT('.$indexValue.'), '.$indexValue.' AS '.$fieldName, 'document_id');
            $this->qb->groupBy($indexValue);
        } else {
    ...
    public function selectIndex($fieldName, $key, $value)
    {
        ...
        $this->select(  ($useDistinct ? 'DISTINCT ' : '' ).$indexAlias.'.'.$key,
                        $indexAlias.'.'.$value);
        return $this;
    }
    ...
    public function where($fieldName, $value = null, $condition = '=', $composite = null)
    {
        if ($fieldName == self::DOCUMENT_TYPE) {
            ...
            return parent::where($fieldName, $value, $condition, $composite);
        }
    }
    ...
    protected function getExpr($fieldName, $value, $condition, $indexData)
    {
        ...
        if ($indexData['indexType']=='fulltext') {
            ...
            return 'MATCH ('.$valueColumn.') AGAINST ('.$valueParam.' IN BOOLEAN MODE)';
        }
    ...
    public function whereLanguageIs($value, $translated=true)
    {
        ...
        if ($translated) {
            $this->qb->andWhere($this->expr->eq($alias.'.'.self::DOCUMENT_DETAIL_TRANSLATED, ':language_translated'))
    ...
    public function showVisible()
    {
        ...
        if ($this->options['type'] == 'PUBLISHED_DRAFT') {
            $expr = $this->expr->orX(
                $this->expr->eq(self::DOCUMENT_DETAIL_TABLE_PUBLISHED_ALIAS.'.'.self::DOCUMENT_DETAIL_IS_VISIBLE, $valueParam),
                $this->expr->eq(self::DOCUMENT_DETAIL_TABLE_DRAFT_ALIAS.'.'.self::DOCUMENT_DETAIL_IS_VISIBLE, $valueParam)
            );
        } else {
            $alias = $this->getDetailTableAlias();
            $expr = $this->expr->eq($this->getDetailTableAlias().'.'.self::DOCUMENT_DETAIL_IS_VISIBLE, $valueParam);
        }
    ...
    private function finalizeQuery()
    {
        ...
        if (!$this->statusSet && !(isset($this->options['type']) && $this->options['type'] == 'PUBLISHED_DRAFT')) {
        ...
        if (__Config::get('pinax.dataAccess.documentVisibility') && !$this->visibilitySet) {
            $this->showVisible();
        }
    }
        ...
        if (empty($this->unionArray)) {
            ...
            if (!empty($this->unionOrderBy)) {
                ...
                if (!$this->ar->getField($fieldName)->isSystemField) {
                    ...
                    $detailAlias = (isset($this->options['type']) && $this->options['type'] == 'PUBLISHED_DRAFT') ? self::DOCUMENT_DETAIL_TABLE_PUBLISHED_ALIAS : self::DOCUMENT_DETAIL_TABLE_ALIAS;
            ...
            try {
                if (__Config::get('QUERY_CACHING') && ($cacheDriver = pinax_dataAccessDoctrine_DataAccess::getCache())) {
                ...
                require_once(pinax_Paths::get('CORE_LIBS').'sql-formatter/lib/SqlFormatter.php');
                $trace = $e->getTrace();
    ...
    {
    	throw new \RuntimeException("RecordIteratorDocument::createCountQuery not implemented");
    }
}

...
define ('K_PATH_CACHE', pinax_Paths::getRealPath( 'CACHE' ) );
define ('K_PATH_URL_CACHE', pinax_Paths::getRealPath( 'CACHE' ) );
define ('K_PATH_IMAGES', pinax_Paths::getRealPath( 'APPLICATION_TEMPLATE' ) );


class pinax_template_layoutManager_PDF extends pinax_template_layoutManager_PHP
{
	private $pageUrl;
	private $pageTitle;
	...
	function __construct($fileName='', $rootPath='')
	{
		...
		$this->pageUrl = PNX_HOST."/index.php?".__Request::get( '__url__' );
	...
	function apply(&$regionContent)
	{
		...
		$pdf = new PinaxPDF( (string)$xml['orientation'] == 'portrait' ? 'P' : 'L' , PDF_UNIT, (string)$xml['size'], true, 'UTF-8', false);
		...
		{
			$logo = str_replace( pinax_Paths::get( 'APPLICATION_TEMPLATE' ), '', $logo );
...
EOF;
	...
	private function fixHtml( &$html )
	{
		...
		$html = str_replace( array( 'href="index.php?', 'href="getFile.php?' ), array( 'href="'.PNX_HOST.'/index.php?', 'href="'.PNX_HOST.'/getFile.php?' ), $html );
	...
	private function imageSrc( $url )
	{
		if ( strpos( $url, 'getImage.php' ) !== false )
		{
			...
			$media = pinaxcms_mediaArchive_MediaManager::getMediaById( $image['id'] );

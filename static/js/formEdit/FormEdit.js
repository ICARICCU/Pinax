...
jQuery( (function( $ ){
    ...
    $.PinaxRegisterType = function (type, typeDef) {

        fieldTypes[type] = typeDef;
    };

    $.PinaxRegisterTypeGet = function (type) {
        return fieldTypes[type];
    };

    $.fn.PinaxFormEdit = function(pinaxOpt) {
        ...
        var $form = jQuery(this),
            ...
            checkRepeatMaxRecords = function (maxRec, rows, $button) {
                if (maxRec > 0 && rows >= maxRec) {
                    $button.addClass('GButtonDisabled').attr('disabled', 'disabled').blur();
                } else {
                    $button.removeClass('GButtonDisabled').removeAttr('disabled');
                }
            }
            ...
            overloadCaller = function (method) {
                ...
                if (fieldTypes[type] && fieldTypes[type][method]) {
                    ...
                    $this.data({overloadCalled: true, formEdit: fieldTypes[type], pinaxOpt: pinaxOpt});
                ...
                $form.validVal({
                        ...
                        fields: {
                            ...
                            filter: function( fields ) {
                                ...
                                $(fields).each(function(index, el){
                                    var $el = $(el);
                                    if ($el.parent().parent().css('display') !== 'none' || $el.parents('fieldset.collapsible').length) {
                    ...
                    fields: {
                        ...
                        onInvalid: function( $form, language ) {
                            var fieldset = $(this).parents('fieldset.collapsible');
                            $(fieldset).children('.control-group').css('display', 'block');
                    ...
                    form: {
                        onValidate: function () {
                            ...
                            if (pinaxOpt.customValidation && typeof(window[pinaxOpt.customValidation]) == 'function') {
                                ...
                                if (error = window[pinaxOpt.customValidation](fieldVals)) {
                        ...
                        onInvalid: function( field_arr, language ) {
                            ...
                            if ($invalidEl.length) {
                                ...
                                Pinax.events.broadcast("pinax.message.showError", {"title": lang.errorValidationMsg, "message": ""});
                            }
                        }
                    }
                });
                            ...
                            $(t).each(function(index, val){
                                ...
                                $container.find("[name]").each(function(){
                                    ...
                                    if ($el.attr("name")==elName) {
                                        if (states[index]=="1") {
                                            $el.closest("div.control-group, div.form-group").show();
                                        } else {
                                            $el.closest("div.control-group, div.form-group").hide();
                                        }
                                    }
                                });

                                if (states[index]=="1") {
                                    $container.find("#"+elName).show().find("[name]").closest("div.control-group, div.form-group").show();
                                } else {
                                    $container.find("#"+elName).hide().find("[name]").closest("div.control-group, div.form-group").hide();
                                }
                            });
        ...
        $form.find('fieldset[data-type=repeat]').each(function () {
            ...
            if (opt.isCollapsable) {
                ...
                $rowContainer
                    .append('<div class="GFERowCollapsed"><div class="GFERowHeader"><span class="GFERowHandler" title="' + lang.drag + '" alt="' + lang.drag + '"></span><span class="GFERecordTitle">' + lang.record + ' <span class="GFERecordId">1</span></span><span class="GFERowPreview"></span></div><div class="GFERowPanel">'+
                            '<a href="#" class=" GFERowEdit" title="' + lang.edit + '" alt="' + lang.edit + '"><span class="btn-icon icon-pencil fa fa-pencil"></span></a>'+
                            '<a href="#" class=" GFERowDelete" title="' + lang.remove + '" alt="' + lang.remove + '"><span class="btn-icon icon-trash fa fa-trash"></span></a>'+
                            '</div><div class="GFESideClearer"></div></div>')
                .children('.GFERowExpanded')
                    .append('<div class="GFERowButtonContainer"><div class="btn-group"><input type="button" value="' + lang.confirm + '" class="btn btn-primary GFERowDoCollapse  GFERowDoConfirm">&nbsp;<input type="button" value="' + lang.cancel + '" class="btn GFERowDoCollapse"></div></div>')
                    .hide();
            }
            else {
                $rowContainer
                    .append('<span class="GFERowHandler GFERowHandlerExpanded" title="' + lang.drag + '" alt="' + lang.drag + '"></span><a href="#" class=" GFERowDelete GFERightIcon" title="' + lang.remove + '" alt="' + lang.remove + '"><span class="btn-icon icon-trash"></span></a>')
            ...
            if (pinaxOpt.formData) {
                repeaterSetValues(pinaxOpt.formData[fieldSetId]);
            }

            Pinax.events.on("pinaxcms.repeater.setValues", function(e) {
            ...
            Pinax.events.on("pinaxcms.fileUpload", function(e) {
                if ($fieldSet.attr('id')!=e.message.targetId) return;
                $footer = $fieldSet.children('.GFEFooter');
            ...
            const $button = $fieldSet.find('.GFEAddRow');
            $rows = $fieldSet.children('.GFERowContainer');
            checkRepeatMaxRecords(opt.maxRec, $rows.length, $button);
        });

        $form.find(':input[name]').each(function () {
            var $el = $(this);
            var type = $el.attr('data-type');
            var value = pinaxOpt.formData[this.name];
            if (!type && value===null) {
                value = '';
            ...
            if ($(this).data('pnx-inRepeater')!==true) {
                overloadCaller.call(this, '__construct');
            }
        });
        ...
        jQuery(document).on('click', '.GFERowDelete', function (e) {
            ...
            if ($rows.length == opt.minRec) {
                Pinax.events.broadcast("pinax.message.showError", {"title": lang.minRecordMsg + opt.minRec, "message": ""});
                return;
            }
            const $button = jQuery(this).parents('.GFEFieldset').find('.GFEAddRow');
            checkRepeatMaxRecords(opt.maxRec != 0 ? opt.maxRec + 1 : 0, $rows.length, $button);
            $container.remove();
        ...
        var setFormButtonStates = function(state) {
            if (state) {
                jQuery('.js-pinaxcms-cancel').removeAttr('disabled');
                jQuery('.js-pinaxcms-save').removeAttr('disabled');
            } else {
                jQuery('.js-pinaxcms-cancel').attr('disabled', 'disabled');
                jQuery('.js-pinaxcms-savecomment').attr('disabled', 'disabled');
                jQuery('.js-pinaxcms-save').attr('disabled', 'disabled');
            }
        }
        var saveOrCancelSuccess = function(data, triggerAction, titleChanged) {
            ...
            if (titleChanged===true) {
                Pinax.events.broadcast("pinaxcms.renameTitle");
            }
        }
        ...
        jQuery('.js-pinaxcms-cancel').click(function (e) {
            e.preventDefault();
            window.onbeforeunload = null;
            setFormButtonStates(false);
            jQuery.ajax(pinaxOpt.AJAXAction, {
        ...
        jQuery('.js-pinaxcms-savecomment').click(function (e){
            enableValidation();
            e.preventDefault();
            var $el = jQuery(this);
            ...
            var pos = Pinax.events.on('pinax.closeDialog', function(e){
                Pinax.events.unbind('pinax.closeDialog', pos);
                if (isCancelled) return;
                $el.val(e.message[newId]);
                jQuery('.js-pinaxcms-save').trigger('click');
                $el.val(undefined);
            })
        ...
        jQuery('.js-pinaxcms-save').click(function (e) {
            Pinax.events.broadcast("pinax.formEdit.prepareSave");
            enableValidation();
            setIndexFields();
            e.preventDefault();
            ...
            if (repeaterMinValuesError) {
                Pinax.events.broadcast("pinax.message.showError", {"title": repeaterMinValuesError, "message": ""});
                return;
            }
            ...
            if (pinaxOpt.AJAXAction.indexOf('js:')===0) {
                window[pinaxOpt.AJAXAction.substr(3)](data);
            } else {
        ...
        if (jQuery('.js-pinaxcms-save').length || jQuery('.js-pinaxcms-savecomment').length) {
            ...
            for (var k in backupData) {
                if (typeof(backupData[k])=='string' && !backupData[k]) {
                    backupData[k] = pinaxOpt.formData[k] ? pinaxOpt.formData[k] : '';
                }
            }
            ...
            window.onbeforeunload = function exitWarning(e) {
                ...
                if (backupData!=JSON.stringify(formData.data)) {
                    var msg = PinaxLocale.FormEdit.interruptProcess;
                    e = e || window.event;

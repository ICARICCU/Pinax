    ...
    __construct: function () {
        ...
        var uploadedFileCount = 0;
         ...
         self.init = function() {
            ...
            self.dropDiv = jQuery('<div id="'+dropId+'" class="pinax-dropzone"><p>'+PinaxLocale.FineUploader.uploadButton+' (' + PinaxLocale.FineUploader.maxsize + ': '+self.el.data('maxlabel')+')'+'</p></div>').insertAfter(self.el);
        ...
        self.dispatchDelayed = function(info) {
            if (!isWaiting) {
                ...
                setTimeout(function () {
                    isWaiting = false;
                    Pinax.events.broadcast("pinaxcms.fileUpload", info);
                    if ( queue.length) {
                        info = queue.pop();

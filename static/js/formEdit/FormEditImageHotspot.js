...
jQuery.PinaxRegisterType('imageHotspot', {
    ...
    __construct: function () {
        ...
        self.el = el;
        self.pinaxOpt = el.data('pinaxOpt');
        self.data = {};
        ...
        self.init = function() {
            ...
            var btnLoadImage = $('<button/>')
                ...
                .text(PinaxLocale.HotSpot.loadImage)
                .click(function(evt) {
                    evt.preventDefault();
            ...
            var btnAddTrack = $('<button/>')
                .css('margin-left', 5)
                .text(PinaxLocale.HotSpot.addPath)
                .click(function(evt) {
                    evt.preventDefault();
                    var id = self.calcId();
        ...
        self.onLoadImageClick = function() {
            var url = self.pinaxOpt.mediaPicker;
            url += '&mediaType=IMAGE';
            Pinax.openIFrameDialog(  PinaxLocale.MediaPicker.imageTitle,
                                    url,
                                    900,
                                    50,
                                    50,
                                    el.data('formEdit').openDialogCallback );
            Pinax.lastMediaPicker = self;
        };

        self.setImage = function(image) {
            if (image) {
                self.image.attr('src',
                        Pinax.tinyMCE_options.urls.imageResizer
                            .replace('#id#', image.id)
                            .replace('#w#', '')
                            .replace('#h#', '')
                    );
        ...
        self.addTrack = function(id) {
            ...
            var label = $('<label/>')
                ...
                .addClass('label')
                .addClass('list-label')
                .text(PinaxLocale.HotSpot.hotspotForm);
            ...
            var opt = $('<option value="rect"/>')
                .text(PinaxLocale.HotSpot.formQuad);
            form.append(opt);
            opt = $('<option value="circle"/>')
                .text(PinaxLocale.HotSpot.formCircle);
            form.append(opt);
            formDiv.append(form);
            ...
            var label = $('<label/>')
                ...
                .addClass('label')
                .addClass('list-label')
                .text(PinaxLocale.HotSpot.hotspotType);
            ...
            opt = $('<option value="linkEx"/>')
                .text(PinaxLocale.HotSpot.linkEx);
            type.append(opt);
            opt = $('<option value="link"/>')
                .text(PinaxLocale.HotSpot.linkInt);
            type.append(opt);
            opt = $('<option value="tooltip"/>')
                .text(PinaxLocale.HotSpot.tooltip);
            type.append(opt);
            typeDiv.append(type);
            ...
            var description = $('<textarea/>')
                ...
                .attr('placeholder', PinaxLocale.HotSpot.description)
            ...
            var label = $('<label/>')
                ...
                .addClass('label')
                .addClass('list-label')
                .text(PinaxLocale.HotSpot.urlEx);
            urlDiv.append(label);
            var url = $('<input/>')
                .attr('id', 'url_'+id)
                .attr('type', 'text')
            ...
            var label = $('<label/>')
                ...
                .addClass('label')
                .addClass('list-label')
                .text(PinaxLocale.HotSpot.urlInt);
            ...
            urlInt.select2({
                ...
                ajax: {
                    url: Pinax.ajaxUrl + "&controllerName=pinaxcms.contents.controllers.autocomplete.ajax.PagePicker",
                    dataType: 'json',
                    quietMillis: 100,
                    data: function(term) {
                        return {
    ...
    openDialogCallback: function() {
        ...
        $frame.load(function () {
            jQuery( "img.js-pinaxMediaPicker", $frame.contents().get(0)).click( function(){
                var $img = jQuery( this );
                Pinax.lastMediaPicker.setImage($img.data("jsonmedia"));
                Pinax.closeIFrameDialog();
            });

            jQuery( ".js-pinaxcmsMediaPicker-noMedia", $frame.contents().get(0)).click( function(){
                Pinax.lastMediaPicker.setImage();
                Pinax.closeIFrameDialog();
            });
        });
    }
